...
$(RESULTS_DIR)/%_md.json: $(INPUT_DIR)/%.json
	...
	@/qfs/projects/BECP/bin/qwait_jl_2 -A animate $(SINGULARITY) exec -B $(PROJECT_DIR)/..:/tmp ./energyplus_$(EPLUS_VERSION).sif energyplus -w /tmp/buildings/$(basename $(<F))_injected_BatchVerification.epw -r -p eplus -d /tmp/buildings/$(basename $(<F))_injected_BatchVerification /tmp/buildings/$(basename $(<F))_injected_BatchVerification.idf
